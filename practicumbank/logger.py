...
class PBRotatingLogHandler(object):
    ...
    def initLogging(self, consoleLogging=True):
        ...
        self.log_file = os.path.join(practicumbank.LOG_DIR, self.log_file)

        self.cur_handler = self._config_handler()

        logging.getLogger('practicumbank').addHandler(self.cur_handler)
        ...
        if consoleLogging:
            ...
            logging.getLogger('practicumbank').addHandler(console)

        logging.getLogger('practicumbank').setLevel(logging.DEBUG)
    ...
    def _rotate_logs(self):
        ...
        sb_logger = logging.getLogger('practicumbank')
...
pb_log_instance = PBRotatingLogHandler('practicumbank.log', NUM_LOGS, LOG_SIZE)

def log(toLog, logLevel=MESSAGE):
    pb_log_instance.log(toLog, logLevel)